...
final class AScroller {
     ...
     *                     be used.
     */
    AScroller(Context context, Interpolator interpolator) {
     ...
     * @hide
     */
    AScroller(Context context, Interpolator interpolator, boolean flywheel) {
    ...
    public float getCurrVelocity() {
        if (Float.isNaN(mScrollerX.mCurrVelocity)) {
            mScrollerX.mCurrVelocity = 0f;
        }
        if (Float.isNaN(mScrollerY.mCurrVelocity)) {
            mScrollerY.mCurrVelocity = 0f;
        }
    ...
    static class SplineOverScroller {
        ...
        private void fitOnBounceCurve(int start, int end, int velocity) {
            ...
            final float fVelocity = velocity;
            final float durationToApex = -fVelocity / mDeceleration;
            final float distanceToApex = fVelocity * fVelocity / 2.0f / Math.abs(mDeceleration);
            ...
            switch (mState) {
                case SPLINE:
                    ...
                    if (mDuration < mSplineDuration) {
                        ...
                        mOver = Math.round(mOver * Math.abs(mVelocity) / (float) mMaximumVelocity);
                        onEdgeReached();
                    } else {
                ...
                case BALLISTIC:
                    mStartTime += mDuration;
                    // Work around: startSpringback uses velocity to extend
                    // the duration of animation, so the springback can be more
                    // smooth
                    startSpringback(mFinal, mStart, mFinal > 0 ? 5000 : -5000);
                    break;
                case CUBIC:
                    return false;
            }
        ...
        boolean update() {
            ...
            switch (mState) {
                ...
                case CUBIC: {
                    ...
                    final float t2 = t * t;
                    final float sign = Math.signum(mVelocity);
                    // Springback: use the formula from DecelerateInterpolator:
                    distance = sign * mOver * (1.0f - Math.pow((1.0f - t), 2 * 2.0f));
